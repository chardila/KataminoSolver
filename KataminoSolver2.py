...
def fitbase(k, b, offset):
    if offset[0] + k.shape[0] > b.shape[0] or offset[1] + k.shape[1] > b.shape[1]:
        return False, None
    k_ope = np.pad(k,
                   [(offset[0], b.shape[0] - k.shape[0] - offset[0]), (offset[1], b.shape[1] - k.shape[1] - offset[1])],
                   'constant')
    return True, k_ope
...
def key_variation(K, isFirst=False):
    ...
    if K_ser in key_pat:
        ...
        if isFirst:
            ...
            pat = [K, np.rot90(K), np.rot90(K, 2), np.rot90(K, 3), np.flipud(K), np.rot90(np.flipud(K)),
                   np.rot90(np.flipud(K), 2), np.rot90(np.flipud(K), 3)]
        pat_uni = []
        for p in pat:
...
def explore(index, base, visited, closed_area):
    if base[index] == 1 or visited[index] == 1:
        ...
        if right_next[1] < base.shape[1] and base[right_next] == 0 and visited[
            right_next] == 0 and right_next not in search_queue:
            search_queue.append(right_next)
        # see below
        below_next = (index[0] + 1, index[1])
        if below_next[0] < base.shape[0] and base[below_next] == 0 and visited[
            below_next] == 0 and below_next not in search_queue:
...
def solve(keys, part, b):
    ...
    for K in key_variation(keys[part], part == 0):
        ...
        row_range = range(0, b.shape[0] - K.shape[0] + 1) if part > 0 else range(0, min(b.shape[0] - K.shape[0] + 1,
                                                                                        math.ceil(b.shape[0] / 2)))
        col_range = range(0, b.shape[1] - K.shape[1] + 1) if part > 0 else range(0, min(b.shape[1] - K.shape[1] + 1,
                                                                                        math.ceil(b.shape[1] / 2)))
